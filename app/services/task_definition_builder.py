...
class TaskDefinitionBuilder:
    ...
    def build_cloud_sync_task(self, repository_name: Optional[str] = None) -> Dict[str, Any]:
        """
        ...
        Returns:
            Task definition dictionary
        """
        name = f"Sync {repository_name} to Cloud" if repository_name else "Sync to Cloud"
    ...
    def build_task_list(
        ...
        Args:
            ...
            cleanup_config_id: ID for prune task from config
            prune_request: Request object for manual prune task
            check_config_id: ID for check task from config  
        ...
        if prune_request:
            tasks.append(self.build_prune_task_from_request(prune_request, repository_name))
        elif cleanup_config_id:
            prune_task = self.build_prune_task_from_config(cleanup_config_id, repository_name)
            if prune_task:
                tasks.append(prune_task)
        
        # Add check task (request takes precedence over config)
        if check_request:
            tasks.append(self.build_check_task_from_request(check_request, repository_name))
        elif check_config_id:
            check_task = self.build_check_task_from_config(check_config_id, repository_name)
            if check_task:
        ...
        if notification_config_id:
            notification_task = self.build_notification_task(notification_config_id, repository_name)
