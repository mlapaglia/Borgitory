...
class VirtualArchiveTree:
        ...
        for part in parts:
            ...
            if part not in current.children:
                ...
                virtual_node = VirtualNode(
                    path=current_path, name=part, type="d", explicit=False
                )
    ...
    def get_directory_contents(self, path: str) -> List[Dict]:
        ...
        results.sort(
            key=lambda x: (not x.get("is_directory", False), x["name"].lower())
        )
...
class ArchiveExplorer:
        ...
        try:
            ...
            if tree.path_needs_loading(path):
                if not path:  # Root level
                    ...
                    await self._load_directory_children(
                        repository, archive_name, path, tree
                    )
            ...
            logger.info(
                f"Returning {len(contents)} items for path '{path}' in archive {archive_name}"
            )
            ...
            logger.error(
                f"Failed to list directory contents for {path} in {archive_name}: {e}"
            )
    ...
    async def _load_root_level(
        ...
        borg_args = ["--json-lines", f"{repository.path}::{archive_name}"]
        ...
        logger.info(
            f"Built tree structure with {len(entries)} total entries, {len(root_items)} root items"
        )
    ...
    async def _load_directory_children(
        ...
        if node and node.children:
            logger.info(
                f"Directory {path} already has {len(node.children)} children in tree"
            )
            return
