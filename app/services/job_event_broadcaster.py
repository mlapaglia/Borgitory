...
class JobEventBroadcaster:
    ...
    def broadcast_event(
        ...
        if sent_count > 0:
            logger.debug(f"Broadcasted {event_type.value} event to {sent_count} clients")
    
    def subscribe_client(
        self,
        client_id: Optional[str] = None,
        send_recent_events: bool = True
    ) -> asyncio.Queue:
            ...
            try:
                ...
                for queue in self._client_queues:
                    ...
                    if queue in self._client_queue_metadata:
                        ...
                        if connected_duration > 3600 and metadata["events_sent"] == 0:  # 1 hour
                            queues_to_remove.append(queue)
            ...
            try:
                if self._client_queues:
                    self.broadcast_event(
                        EventType.KEEPALIVE,
                        data={"message": "keepalive", "client_count": len(self._client_queues)}
                    )
