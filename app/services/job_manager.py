...
class BorgJobManager:
            ...
            for i, task in enumerate(job.tasks):
                ...
                try:
                    ...
                    if success:
                        ...
                        # Update database task status
                        self._update_composite_task_status(job_id, i, 'completed', return_code=0)
    ...
    def _update_composite_task_status(self, job_id: str, task_index: int, status: str, error: str = None, return_code: int = None):
        ...
        try:
            ...
            try:
                ...
                if task:
                    ...
                    db.commit()
                    logger.info(f"📝 Updated database task {task.id} status to {status}")
            finally:
                db.close()
    ...
    async def _execute_backup_task(self, job: BorgJob, task: BorgJobTask, task_index: int) -> bool:
        ...
        try:
            ...
            logger.info(f"🔄 Backup settings - Source: {source_path}, Compression: {compression}, Dry run: {dry_run}")
            ...
            return await self._execute_borg_command_with_streaming(command, env, job, task, task_index)
    ...
    async def _execute_check_task(self, job: BorgJob, task: BorgJobTask, task_index: int) -> bool:
        ...
        try:
            ...
            if params.get('verify_data') and params.get('check_type') != "repository_only":
    ...
    async def _execute_cloud_sync_task(self, job: BorgJob, task: BorgJobTask, task_index: int) -> bool:
        ...
        try:
            ...
            logger.info(f"☁️ Cloud sync task - would sync repository {job.repository.name}")
