...
class RcloneService:
    ...
    async def sync_repository_to_sftp(
        ...
        try:
            ...
            yield {
                "type": "started",
                "command": " ".join([c for c in command if not c.startswith("--sftp-pass")]),  # Hide password
                "pid": process.pid
            }
    ...
    async def test_sftp_connection(
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                elif "authentication failed" in error_message or "permission denied" in error_message:
                    return {
                        "status": "failed",
