...
async def create_cloud_sync_config(
    config: CloudSyncConfigCreate, db: Session = Depends(get_db)
):
    """Create a new cloud sync configuration"""
    # Check if name already exists
    existing = (
        db.query(CloudSyncConfig).filter(CloudSyncConfig.name == config.name).first()
    )
...
def get_cloud_sync_configs_html(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        configs_raw = (
            db.query(CloudSyncConfig).order_by(CloudSyncConfig.created_at.desc()).all()
        )
...
def get_cloud_sync_config(config_id: int, db: Session = Depends(get_db)):
    ...
    config = db.query(CloudSyncConfig).filter(CloudSyncConfig.id == config_id).first()
...
async def update_cloud_sync_config(
    config_id: int, config_update: CloudSyncConfigUpdate, db: Session = Depends(get_db)
):
