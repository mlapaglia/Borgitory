...
@router.post("/backup")
async def create_backup(backup_request: BackupRequest, db: Session = Depends(get_db)):
...
@router.post("/prune")
async def create_prune_job(prune_request: PruneRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to start prune job: {str(e)}"
        )


@router.post("/check")
async def create_check_job(check_request: CheckRequest, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if check_request.check_config_id:
            ...
            check_config = (
                db.query(RepositoryCheckConfig)
                .filter(RepositoryCheckConfig.id == check_request.check_config_id)
                .first()
            )
...
@router.get("/")
def list_jobs(
    skip: int = 0, limit: int = 100, type: str = None, db: Session = Depends(get_db)
):
...
async def get_job_output(
    job_id: str, last_n_lines: int = 100, db: Session = Depends(get_db)
):
    ...
    if job and job.is_composite():
        ...
        output = await borg_job_manager.get_job_output_stream(
