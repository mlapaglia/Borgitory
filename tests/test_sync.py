...
class TestSyncEndpoints:
    ...
    async def test_sync_repository_success(
        ...
        try:
            response = await async_client.post(
                "/api/sync/sync", json=sample_sync_request
            )
    ...
    async def test_sync_repository_not_found(
        self, async_client: AsyncClient, sample_sync_request
    ):
        """Test sync request with non-existent repository."""
        response = await async_client.post("/api/sync/sync", json=sample_sync_request)
    ...
    async def test_sync_repository_invalid_data(self, async_client: AsyncClient):
        ...
        response = await async_client.post("/api/sync/sync", json=invalid_request)
...
class TestSyncRepositoryTask:
    ...
    @pytest.mark.asyncio
    async def test_sync_repository_task_missing_records(
        self, test_db, mock_rclone_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_repository_task_rclone_error(
        self, test_db, mock_rclone_service
    ):
