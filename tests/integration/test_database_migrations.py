...
def test_migration_command_succeeds(migration_env, temp_data_dir):
    ...
    # Should succeed
    assert result.returncode == 0, f"Migration failed with exit code {result.returncode}"
...
def test_migration_command_is_idempotent(migration_env, temp_data_dir):
    ...
    assert result1.returncode == 0, f"First migration failed with exit code {result1.returncode}"
    ...
    assert result2.returncode == 0, f"Second migration failed with exit code {result2.returncode}"
...
def test_migration_with_missing_data_dir(migration_env):
    ...
    migration_env["BORGITORY_DATABASE_URL"] = f"sqlite:///{nonexistent_dir}/test_borgitory.db"
...
def test_borgitory_command_available():
    ...
    assert result.returncode == 0, f"borgitory --help failed with exit code {result.returncode}"
...
def test_borgitory_version_command():
    ...
    assert result.returncode == 0, f"borgitory --version failed with exit code {result.returncode}"