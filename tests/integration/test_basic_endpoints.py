...
def test_auth_check_users_endpoint(app_runner_module):
    ...
    response = requests.get(f"{app_runner_module.base_url}/auth/check-users", timeout=10)
    ...
    form_indicators = ["<form", "input", "button", "login", "register", "username", "password"]
...
def test_login_endpoint_post(app_runner_module):
    ...
    response = requests.post(
        f"{app_runner_module.base_url}/auth/login",
        data=login_data,
        timeout=10
    )
...
def test_register_endpoint_post(app_runner_module):
    ...
    response = requests.post(
        f"{app_runner_module.base_url}/auth/register",
        data=register_data,
        timeout=10
    )
...
def test_api_endpoints_return_proper_content_types(app_runner_module):
    ...
    endpoint_tests = [
        ...
        ("/api/debug/info", ["application/json", "text/html"]),  # Might redirect to login (HTML)
    ]
    ...
    for endpoint, expected_types in endpoint_tests:
        ...
        type_match = any(expected_type in content_type for expected_type in expected_types)
        assert type_match, \
    ...
    for method, path, *args in invalid_requests:
        ...
        try:
            if method == "GET":
                ...
                response = requests.post(f"{app_runner_module.base_url}{path}", data=data, timeout=5)
            elif method == "PUT":
                response = requests.put(f"{app_runner_module.base_url}{path}", timeout=5)
            else:
                continue
