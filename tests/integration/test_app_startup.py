...
class AppRunner:
    ...
    def is_ready(self) -> bool:
        ...
        try:
            ...
            return response.status_code in [200, 401, 403]  # Any response means app is running
...
def test_app_serves_main_page(app_runner_module):
    ...
    assert response.status_code in [200, 302, 401, 403], f"Unexpected status code: {response.status_code}"
...
def test_app_handles_auth_check_endpoint(app_runner_module):
    ...
    assert response.status_code == 200, f"Auth check failed with status: {response.status_code}"
    ...
    assert any(keyword in response_text for keyword in ["form", "input", "login", "register"]), \
...
def test_app_startup_with_fresh_database(app_runner):
    ...
    assert app_runner.start(timeout=30), "Application failed to start with fresh database"
    
    # Verify app is responsive
    response = requests.get(f"{app_runner.base_url}/auth/check-users", timeout=5)
    assert response.status_code == 200, "App not responsive after fresh database creation"
