...
class TestCronDescriptionService:
    ...
    def test_get_human_description_valid_expressions(self):
        ...
        for cron_expr, expected_desc in test_cases:
            ...
            assert result["error"] is None, f"Unexpected error for '{cron_expr}': {result['error']}"
            assert result["description"] is not None, f"No description for '{cron_expr}'"
            # Basic validation that we got a reasonable description
            assert len(result["description"]) > 5, f"Description too short for '{cron_expr}'"
    ...
    def test_get_human_description_invalid_expressions(self):
        ...
        for invalid_expr in invalid_expressions:
            ...
            assert result["description"] is None, f"Expected no description for '{invalid_expr}'"
    ...
    def test_get_human_description_empty_input(self):
        ...
        for empty_input in empty_inputs:
            ...
            assert result["description"] is None, f"Expected no description for empty input: '{empty_input}'"
            assert result["error"] is None, f"Expected no error for empty input: '{empty_input}'"
    ...
    def test_get_human_description_whitespace_handling(self):
        ...
        assert "12:00 PM" in result["description"], "Expected proper time parsing"
    ...
    def test_validate_cron_expression_valid(self):
        ...
        for expr in valid_expressions:
            assert CronDescriptionService.validate_cron_expression(expr) is True, f"Expected '{expr}' to be valid"
    ...
    def test_validate_cron_expression_invalid(self):
        ...
        for expr in invalid_expressions:
            assert CronDescriptionService.validate_cron_expression(expr) is False, f"Expected '{expr}' to be invalid"

    def test_validate_cron_expression_none(self):
        """Test validation with None input."""
        assert CronDescriptionService.validate_cron_expression(None) is False
