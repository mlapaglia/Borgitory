...
def mock_command_builder():
    ...
    mock.build_list_archive_contents_command = Mock(return_value=(["borg", "list"], {"BORG_PASSPHRASE": "test"}))
    mock.build_extract_command = Mock(return_value=(["borg", "extract"], {"BORG_PASSPHRASE": "test"}))
    mock.build_repo_info_command = Mock(return_value=(["borg", "info"], {"BORG_PASSPHRASE": "test"}))
    return mock
...
class TestArchiveManager:
    ...
    def test_init_with_dependencies(self):
        ...
        manager = ArchiveManager(job_executor=mock_executor, command_builder=mock_builder)
    ...
    @pytest.mark.asyncio
    async def test_list_archive_contents_success(self, archive_manager, test_repository, mock_process_result, mock_job_executor):
        ...
        # Execute
        result = await archive_manager.list_archive_contents(test_repository, "test-archive")
    ...
    @pytest.mark.asyncio
    async def test_list_archive_contents_command_building_error(self, archive_manager, test_repository, mock_command_builder):
        """Test error in command building."""
        mock_command_builder.build_list_archive_contents_command.side_effect = Exception("Command build error")
    ...
    @pytest.mark.asyncio
    async def test_list_archive_contents_borg_error(self, archive_manager, test_repository, mock_job_executor):
    ...
    @pytest.mark.asyncio
    async def test_list_archive_contents_invalid_json(self, archive_manager, test_repository, mock_job_executor):
    ...
    @pytest.mark.asyncio
    async def test_list_archive_directory_contents_success(self, archive_manager, test_repository):
        """Test listing directory contents using FUSE mount."""
        with patch('app.services.archive_mount_manager.get_archive_mount_manager') as mock_get_manager:
            ...
            mock_mount_manager.mount_archive.assert_called_once_with(test_repository, "test-archive")
            mock_mount_manager.list_directory.assert_called_once_with(test_repository, "test-archive", "/data")
    ...
    async def test_extract_file_stream_success(self, archive_manager, test_repository):
        ...
        with patch('asyncio.create_subprocess_exec') as mock_subprocess:
            # Mock process
            mock_process = Mock()
            mock_process.stdout.read = AsyncMock(side_effect=[b'chunk1', b'chunk2', b''])
    ...
    @pytest.mark.asyncio
    async def test_extract_file_stream_process_cleanup(self, archive_manager, test_repository):
    ...
    @pytest.mark.asyncio
    async def test_get_archive_metadata_success(self, archive_manager, test_repository, mock_job_executor):
        ...
        metadata = await archive_manager.get_archive_metadata(test_repository, "test-archive")
    ...
    @pytest.mark.asyncio
    async def test_get_archive_metadata_not_found(self, archive_manager, test_repository, mock_job_executor):
        """Test archive metadata when archive is not found."""
        # Setup mock result with different archives
        repo_info = {
            "archives": [
                {"name": "other-archive", "size": 1000}
            ]
        }
        ...
        metadata = await archive_manager.get_archive_metadata(test_repository, "nonexistent-archive")
        
        assert metadata is None

    @pytest.mark.asyncio
    async def test_get_archive_metadata_borg_error(self, archive_manager, test_repository, mock_job_executor):
    ...
    def test_find_entries_by_pattern_case_sensitive(self, archive_manager):
        ...
        matches = archive_manager.find_entries_by_pattern(entries, "Test", case_sensitive=True)
    ...
    def test_find_entries_by_pattern_case_insensitive(self, archive_manager):
        ...
        entries = [
            {"path": "Test.txt", "name": "Test.txt"},
            {"path": "test.log", "name": "test.log"},
            {"path": "data/config.cfg", "name": "config.cfg"}
        ]
        
        matches = archive_manager.find_entries_by_pattern(entries, "test", case_sensitive=False)
    ...
    def test_validate_archive_path_valid(self, archive_manager):
        ...
        with patch('app.services.archive_manager.validate_archive_name') as mock_validate_archive:
            with patch('app.services.archive_manager.sanitize_path') as mock_sanitize_path:
                mock_validate_archive.return_value = None  # No exception means valid
                mock_sanitize_path.return_value = "/safe/path"
                
                errors = archive_manager.validate_archive_path("valid-archive", "/safe/path")
    ...
    def test_validate_archive_path_invalid(self, archive_manager):
        ...
        with patch('app.services.archive_manager.validate_archive_name') as mock_validate_archive:
            with patch('app.services.archive_manager.sanitize_path') as mock_sanitize_path:
                ...
                errors = archive_manager.validate_archive_path("../evil", "../../etc/passwd")
