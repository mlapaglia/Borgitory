...
class TestCronDescriptionAPI:
    ...
    def test_describe_cron_expression_valid(self, client):
        ...
        form_data = {"custom_cron_input": "0 12 * * *"}
        
        response = client.post("/api/schedules/cron/describe", data=form_data)
    ...
    def test_describe_cron_expression_invalid(self, client):
        ...
        form_data = {"custom_cron_input": "invalid cron"}
    ...
    def test_describe_cron_expression_complex_valid(self, client):
        ...
        for cron_expr in test_cases:
            form_data = {"custom_cron_input": cron_expr}
            
            response = client.post("/api/schedules/cron/describe", data=form_data)
            
            assert response.status_code == 200, f"Failed for expression: {cron_expr}"
            html_content = response.text
            assert "Schedule:" in html_content, f"No schedule description for: {cron_expr}"
    ...
    def test_concurrent_requests(self, client):
        ...
        for expr, status_code, content in results:
            ...
            if expr == "invalid":
                ...
                assert "Schedule:" in content or len(content.strip()) < 50  # Valid or empty
    ...
    def test_service_error_handling(self, mock_service, client):
        ...
        form_data = {"custom_cron_input": "invalid"}
        response = client.post("/api/schedules/cron/describe", data=form_data)
    ...
    def test_form_data_parsing_edge_cases(self, client):
        ...
        edge_cases = [
            ...
            {"custom_cron_input": "0 12 * * *", "extra_field": "ignored"},  # Extra fields
        ]
