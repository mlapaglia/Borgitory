...
class TestJobStreamingFixes:
    ...
    @pytest.mark.asyncio
    async def test_task_streaming_sends_individual_lines(self, job_stream_service, mock_job_manager, mock_composite_job):
    ...
    @pytest.mark.asyncio
    async def test_task_streaming_handles_new_output_events(self, job_stream_service, mock_job_manager, mock_composite_job):
...
class TestStreamingEfficiency:
    ...
    def test_individual_line_streaming_reduces_data_transfer(self):
        ...
        assert accumulated_size > len(individual_events[0])  # Single accumulated is larger than single line
