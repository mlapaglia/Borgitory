...
class TestCloudBackupCoordinator:
    ...
    def test_coordinator_initialization_with_custom_params(
        self, mock_db_session_factory, mock_rclone_service
    ):
    ...
    @pytest.mark.asyncio
    async def test_should_trigger_cloud_backup_enabled_config(
        self, coordinator, mock_db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_should_trigger_cloud_backup_disabled_config(
        self, coordinator, mock_db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_should_trigger_cloud_backup_config_not_found(
        self, coordinator, mock_db_session
    ):
    ...
    @pytest.mark.asyncio
    async def test_should_trigger_cloud_backup_no_rclone_service(
        self, mock_db_session_factory
    ):
        """Test _should_trigger_cloud_backup without rclone service."""
        coordinator = CloudBackupCoordinator(
            db_session_factory=mock_db_session_factory, rclone_service=None
        )
    ...
    @pytest.mark.asyncio
    async def test_trigger_cloud_backup_success(
        self, coordinator, mock_db_session, sample_repository_data
    ):
        ...
        with patch.object(coordinator, "_execute_cloud_backup", new_callable=AsyncMock):
            task_id = await coordinator.trigger_cloud_backup(
                sample_repository_data, 1, source_job_id=123
            )
    ...
    @pytest.mark.asyncio
    async def test_trigger_cloud_backup_not_eligible(
        self, coordinator, mock_db_session, sample_repository_data
    ):
    ...
    @pytest.mark.asyncio
    async def test_execute_rclone_backup_success(
        self,
        coordinator,
        mock_rclone_service,
        ...
        task = CloudBackupTask(
            task_id="test_task_1", repository_id=1, cloud_sync_config_id=1
        )

        await coordinator._execute_rclone_backup(
            task, sample_repository_data, sample_cloud_config
        )
    ...
    @pytest.mark.asyncio
    async def test_execute_rclone_backup_failure(
        self,
        coordinator,
        mock_rclone_service,
        sample_repository_data,
    ...
    @pytest.mark.asyncio
    async def test_execute_cloud_backup_success(
        self, coordinator, mock_db_session, sample_repository_data, sample_cloud_config
    ):
        ...
        with patch.object(
            coordinator, "_get_cloud_sync_config", return_value=sample_cloud_config
        ) as mock_get_config, patch.object(
            coordinator, "_execute_rclone_backup", new_callable=AsyncMock
        ) as mock_rclone, patch.object(
            coordinator, "_update_cloud_backup_status", new_callable=AsyncMock
        ) as mock_update:
    ...
    @pytest.mark.asyncio
    async def test_execute_cloud_backup_failure(
        self, coordinator, mock_db_session, sample_repository_data
    ):
        ...
        with patch.object(
            coordinator, "_get_cloud_sync_config", return_value=None
        ), patch.object(
            coordinator, "_update_cloud_backup_status", new_callable=AsyncMock
        ) as mock_update:
            await coordinator._execute_cloud_backup(task, sample_repository_data)
