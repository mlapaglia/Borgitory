...
class TestJobServicesPureDI:
    ...
    def test_job_services_no_longer_singleton(self):
        ...
        # Should be different instances (no longer singleton)
        assert service3 is not service4, "JobStreamService should no longer be singleton"
...
class TestJobServicesDIIntegration:
    ...
    def test_job_services_can_be_mocked_completely(self):
        ...
        with override_dependency(get_job_render_service, lambda: mock_render_service) as client:
        ...
        with override_dependency(get_job_stream_service, lambda: mock_stream_service) as client:
            # The override should be active
    ...
    def test_job_services_di_isolation(self):
        ...
        # First context
        with override_dependency(get_job_render_service, lambda: mock_render1) as client1:
            response1 = client1.get("/api/debug/info")
            assert response1 is not None
            
        # Second context (should be isolated)
        with override_dependency(get_job_render_service, lambda: mock_render2) as client2:
