...
class TestRepositoryParser:
    ...
    @patch('os.listdir')
    def test_parse_borg_config_valid_encrypted(self, mock_listdir, mock_exists, repository_parser):
    ...
    @patch('os.path.exists')
    @patch('builtins.open')
    def test_parse_borg_config_invalid_config(self, mock_open_func, mock_exists, repository_parser):
        """Test parsing invalid config file."""
        mock_exists.return_value = True
        mock_open_func.return_value.__enter__.return_value.read.return_value = "[invalid]\nnotrepo = true\n"
    ...
    @patch('os.path.exists')
    @patch('builtins.open')
    def test_parse_borg_config_exception_handling(self, mock_open_func, mock_exists, repository_parser):
    ...
    async def test_start_repository_scan_default_path(self, repository_parser):
        ...
        with patch('app.services.repository_parser.get_job_manager') as mock_get_manager:
    ...
    async def test_get_scan_results_success(self, repository_parser):
        ...
        with patch('app.services.repository_parser.get_job_manager') as mock_get_manager:
            ...
            repository_parser._parse_scan_output = AsyncMock(return_value=[
            ...
            repository_parser._parse_scan_output.assert_called_once_with("/repo1\n/repo2\n")
    ...
    async def test_get_scan_results_with_errors(self, repository_parser):
        ...
        with patch('app.services.repository_parser.get_job_manager') as mock_get_manager:
            ...
            repository_parser._parse_scan_output = AsyncMock(return_value=[{"name": "repo1"}])
    ...
    async def test_parse_scan_output_success(self, mock_exists, repository_parser):
        ...
        repository_parser.parse_borg_config = Mock(return_value={
            "mode": "encrypted",
        ...
        repository_parser._get_repository_metadata = AsyncMock(return_value={
            "size": "1.2G",
            "last_backup": "2023-01-01T12:00:00Z"
        })
    ...
    @pytest.mark.asyncio
    @patch('os.path.exists')
    async def test_parse_scan_output_invalid_repos(self, mock_exists, repository_parser):
    ...
    @pytest.mark.asyncio
    @patch('os.path.exists')
    async def test_parse_scan_output_nonexistent_path(self, mock_exists, repository_parser):
    ...
    @pytest.mark.asyncio
    async def test_get_repository_metadata_success(self, repository_parser, mock_command_runner):
        ...
        with patch('os.path.exists', return_value=True):
            with patch('os.walk') as mock_walk:
                ...
                with patch('os.path.getmtime', return_value=1640995200.0):  # 2022-01-01 00:00:00
    ...
    @pytest.mark.asyncio
    async def test_get_repository_metadata_du_error(self, repository_parser, mock_command_runner):
    ...
    @pytest.mark.asyncio
    async def test_get_repository_metadata_no_data_dir(self, repository_parser, mock_command_runner):
    ...
    @pytest.mark.asyncio
    async def test_verify_repository_access_success(self, repository_parser, test_repository):
        """Test successful repository access verification."""
        with patch('app.services.repository_parser.build_secure_borg_command') as mock_build_cmd:
            mock_build_cmd.return_value = (["borg", "info"], {"BORG_PASSPHRASE": "test"})
            
            with patch('app.services.repository_parser.get_job_manager') as mock_get_manager:
                mock_job_manager = Mock()
                mock_job_manager.start_borg_command = AsyncMock(return_value="verify-job")
                mock_job_manager.get_job_status.return_value = {
                    "completed": True,
                    "status": "completed",
                    "error": None,
    ...
    @pytest.mark.asyncio
    async def test_verify_repository_access_wrong_passphrase(self, repository_parser, test_repository):
        """Test repository access verification with wrong passphrase."""
        with patch('app.services.repository_parser.build_secure_borg_command') as mock_build_cmd:
            mock_build_cmd.return_value = (["borg", "info"], {"BORG_PASSPHRASE": "wrong"})
    ...
    @pytest.mark.asyncio
    async def test_verify_repository_access_not_exists(self, repository_parser, test_repository):
    ...
    @pytest.mark.asyncio
    async def test_verify_repository_access_timeout(self, repository_parser, test_repository):
    ...
    @pytest.mark.asyncio
    async def test_verify_repository_access_security_error(self, repository_parser, test_repository):
    ...
    @pytest.mark.asyncio
    async def test_verify_repository_access_job_not_found(self, repository_parser, test_repository):
    ...
    async def test_scan_for_repositories_legacy_method(self, repository_parser):
        ...
        repository_parser.get_scan_results = AsyncMock(return_value=[
        ...
        status_responses = [
            ...
            {"completed": False, "status": "running", "output": "", "error": None},
            {"completed": True, "status": "completed", "output": "/tmp/test-repo", "error": None}
        ]
    ...
    async def test_scan_for_repositories_with_error(self, repository_parser):
        ...
        repository_parser.start_repository_scan = AsyncMock(return_value="scan-job")
        repository_parser.check_scan_status = AsyncMock(return_value={
            "completed": False,
            "status": "failed",
            "output": "",
            "error": "Scan failed"
    ...
    @pytest.mark.asyncio
    async def test_verify_repository_access_with_custom_passphrase(self, repository_parser, test_repository):
        """Test repository verification with custom passphrase."""
        with patch('app.services.repository_parser.build_secure_borg_command') as mock_build_cmd:
            mock_build_cmd.return_value = (["borg", "info"], {"BORG_PASSPHRASE": "custom"})
            ...
            with patch('app.services.repository_parser.get_job_manager') as mock_get_manager:
                ...
                result = await repository_parser.verify_repository_access(
                    test_repository, 
                    test_passphrase="custom-passphrase"
                )
