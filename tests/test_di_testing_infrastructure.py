...
class TestDependencyOverrideInfrastructure:
    ...
    def test_single_dependency_override(self):
        """Test that single dependency override works."""
        mock_debug_service = MockServiceFactory.create_mock_debug_service()
        
        with override_dependency(get_debug_service, lambda: mock_debug_service) as client:
...
class TestMockServiceFactory:
    ...
    def test_create_mock_borg_service(self):
        ...
        assert mock.create_backup.return_value == {"job_id": "test-job-123"}
        assert mock.list_archives.return_value == []
        assert mock.verify_repository_access.return_value is True
    
    def test_create_mock_debug_service(self):
    ...
    def test_create_mock_job_render_service(self):
    ...
    def test_create_mock_archive_manager(self):
...
class TestDependencyTestHelper:
    ...
    def test_verify_service_creation(self):
    ...
    def test_verify_service_creation_failure(self):
    ...
    def test_verify_dependency_override_works(self):
...
class TestIntegrationWithExistingTests:
    ...
    def test_can_still_call_services_directly(self):
    ...
    def test_direct_calls_vs_dependency_overrides(self):
        """Test the difference between direct calls and dependency overrides."""
        # Direct call gets the real service
        real_service = get_debug_service()
        
        # Override gets the mock
        mock_service = MockServiceFactory.create_mock_debug_service()
        with override_dependency(get_debug_service, lambda: mock_service):
            # Direct call still gets real service (not affected by FastAPI overrides)
            direct_call_service = get_debug_service()
            assert direct_call_service is real_service  # Still the real service
            
            # But FastAPI DI context would use the mock (tested via API calls)
            # This is why we need to update tests to use FastAPI's DI system
