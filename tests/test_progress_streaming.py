...
class TestProgressStreaming:
    ...
    async def test_progress_streaming_basic_flow(self, mock_repository, mock_db):
        ...
        try:
            ...
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
                ...
                progress_bar_events = [e for e in events if e['event'] == 'progress-bar']
                complete_events = [e for e in events if e['event'] == 'complete']
                
                # Should have multiple progress messages (at least a few)
                assert len(progress_events) >= 3, f"Should have at least 3 progress messages, got {len(progress_events)}"
                
                # Should have corresponding progress bar updates
                assert len(progress_bar_events) >= 3, f"Should have at least 3 progress bar updates, got {len(progress_bar_events)}"
                
                # Should have completion event
                assert len(complete_events) == 1, "Should have exactly 1 completion event"
                ...
                if percentages:
                    assert percentages == sorted(percentages), "Progress percentages should be increasing"
    ...
    async def test_progress_streaming_error_handling(self, mock_repository, mock_db):
        ...
        try:
            async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as ac:
                ...
                assert len(progress_events) >= 1, f"Should have initial progress messages, got {len(progress_events)}"
    ...
    def test_progress_streaming_html_template_elements(self):
        ...
        assert 'sse-connect="/api/repositories/1/stats/progress"' in html_content, "Should connect to progress endpoint"
        
        # Verify SSE swap elements
        assert 'sse-swap="progress"' in html_content, "Should have progress message swap"
        assert 'sse-swap="progress-bar"' in html_content, "Should have progress bar swap"
        ...
        assert 'id="progress-bar-inner"' in html_content, "Should have progress bar inner element"
        assert 'bg-gray-200 dark:bg-gray-700 rounded-full h-2' in html_content, "Should have progress track"
