...
class CloudBackupCoordinator:
    ...
    async def trigger_cloud_backup(
        ...
        future = asyncio.create_task(self._execute_cloud_backup(task, repository_data))
    ...
    async def _should_trigger_cloud_backup(
        self, repository_id: int, cloud_sync_config_id: int
    ) -> bool:
        """Check if cloud backup should be triggered"""
        try:
            ...
            with self.db_session_factory() as db:
                ...
                if not config:
                    logger.warning(
                        f"Cloud sync config {cloud_sync_config_id} not found"
                    )
                    return False
    ...
    async def _execute_cloud_backup(
        self, task: CloudBackupTask, repository_data: Dict[str, Any]
    ) -> None:
        ...
        async with self._upload_semaphore:
            try:
                ...
                cloud_config = await self._get_cloud_sync_config(
                    task.cloud_sync_config_id
                )
                if not cloud_config:
                ...
                if self.rclone_service:
                    await self._execute_rclone_backup(
                        task, repository_data, cloud_config
                    )
                else:
    ...
    async def _execute_rclone_backup(
        ...
        try:
            ...
            if not result["success"]:
                raise Exception(
                    f"Rclone backup failed: {result.get('error', 'Unknown error')}"
                )
    ...
    async def _update_cloud_backup_status(self, task: CloudBackupTask) -> None:
        ...
        try:
            ...
            from models.database import CloudBackupJob
