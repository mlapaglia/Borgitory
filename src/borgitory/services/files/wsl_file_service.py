...
class WSLFileService(FileServiceProtocol):
    ...
    async def remove_file(self, file_path: str) -> None:
        ...
        result = await self.command_executor.execute_command(
            command=["rm", "-f", file_path], timeout=30.0
        )
    ...
    async def open_file(self, file_path: str, mode: str) -> IO[bytes]:
        ...
        if mode == "rb":
            ...
            result = await self.command_executor.execute_command(
                command=["cat", file_path], timeout=30.0
            )

            if not result.success:
                raise FileNotFoundError(
                    f"Failed to read file {file_path}: {result.stderr}"
                )
            ...
            raise NotImplementedError(
                f"WSL file service does not support mode '{mode}'. Use write_file() for writing."
            )

    async def exists(self, file_path: str) -> bool:
        """Check if a file or directory exists at the given path using WSL."""
        result = await self.command_executor.execute_command(
            command=["test", "-e", file_path], timeout=10.0
        )
    ...
    async def isfile(self, file_path: str) -> bool:
        ...
        result = await self.command_executor.execute_command(
            command=["test", "-f", file_path], timeout=10.0
        )
        return result.success
