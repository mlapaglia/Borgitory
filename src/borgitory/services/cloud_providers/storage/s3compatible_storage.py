...
This module provides S3-compatible storage operations with clean separation
...
class S3CompatibleStorageConfig(CloudStorageConfig):
    ...
    endpoint_url: str = Field(..., min_length=1, description="S3-compatible endpoint URL")
    ...
    def validate_access_key(cls, v: str) -> str:
        ...
        if len(v) < 16:
            raise ValueError("Access Key must be at least 16 characters long")
    ...
    def validate_secret_key(cls, v: str) -> str:
        ...
        if len(v) < 40:
            raise ValueError("Secret Key must be at least 40 characters long")
        if not re.match(r"^[A-Za-z0-9+/=]+$", v):
...
class S3CompatibleStorage(CloudStorage):
    ...
    This class handles S3-compatible operations while maintaining the clean
    ...
    def __init__(
        ...
        Args:
            config: Validated S3-Compatible configuration
        """
        self._config = config
    ...
    async def upload_repository(
        ...
        """Upload repository to S3-Compatible"""
        if progress_callback:
            progress_callback(
                SyncEvent(
                    type=SyncEventType.STARTED,
                    message=f"Starting S3-Compatible upload to bucket {self._config.bucket_name}",
                )
            )

        try:
            final_status = None
            async for progress in self.sync_repository_to_s3compatible(
            ...
            if final_status == "failed":
                raise Exception("S3-Compatible sync failed with non-zero exit code")

            if progress_callback:
                progress_callback(
                    SyncEvent(
                        type=SyncEventType.COMPLETED,
                        message="S3-Compatible upload completed successfully",
                    )
                )

        except Exception as e:
            error_msg = f"S3-Compatible upload failed: {str(e)}"
            if progress_callback:
                progress_callback(
    ...
    async def test_connection(self) -> bool:
        ...
        try:
            result = await self.test_s3compatible_connection(
    ...
    def get_connection_info(self) -> ConnectionInfo:
        """Get S3-Compatible connection info for display"""
    ...
    def get_rclone_mapping(cls) -> RcloneMethodMapping:
        ...
        return RcloneMethodMapping(
            ...
            optional_params={"path_prefix": "", "region": ""},
        )

    def _build_s3compatible_flags(
        self,
        access_key_id: str,
        ...
        """Build S3-Compatible configuration flags for rclone command"""
        flags = [
            "--s3-access-key-id",
            access_key_id,
            ...
            secret_access_key,
            "--s3-provider",
            "Other",  # Use "Other" for S3-compatible providers
            "--s3-storage-class",
            storage_class,
            "--s3-endpoint",
            endpoint_url,
        ...
        # Only add region if it's not empty (for S3-compatible providers, region is often empty)
        if region:
            flags.extend(["--s3-region", region])

        return flags

    async def sync_repository_to_s3compatible(
        self,
        repository_path: str,
        path_prefix: str = "",
    ) -> AsyncGenerator[ProgressData, None]:
        """Sync a Borg repository to S3-Compatible using Rclone with direct S3 backend"""
        ...
        s3_flags = self._build_s3compatible_flags(
    ...
    async def test_s3compatible_connection(
        self,
        access_key_id: str,
        secret_access_key: str,
        bucket_name: str,
        ...
        """Test S3-Compatible connection by checking bucket access"""
        try:
            ...
            # Add S3 configuration flags
            s3_flags = self._build_s3compatible_flags(
                access_key_id, secret_access_key, endpoint_url, region, storage_class
            )
            command.extend(s3_flags)
            ...
            if result.success:
                test_result = await self._test_s3compatible_write_permissions()
    ...
    async def _test_s3compatible_write_permissions(self) -> ConnectionTestResult:
        ...
        try:
            ...
            async with self._file_service.create_temp_file(
                ...
                s3_flags = self._build_s3compatible_flags(
                    self._config.access_key, self._config.secret_key, self._config.endpoint_url,
                    self._config.region, self._config.storage_class
                )
...
@register_provider(
    name="s3compatible",
    label="S3-Compatible",
    description="S3-compatible storage providers (Hetzner, MinIO, etc.)",
...
class S3CompatibleProvider:
    """S3-Compatible provider registration"""

    config_class = S3CompatibleStorageConfig
    storage_class = S3CompatibleStorage