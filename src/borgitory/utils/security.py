...
def build_secure_borg_command(
    ...
    # Start from the current environment so PATH and other vars are preserved.
    # create_subprocess_exec replaces the process env entirely when env= is set.
...
def build_secure_borg_command_with_keyfile(
    ...
    return BorgCommandResult(command=command, environment=env)
...
def create_borg_command(
    ...
    """
    return build_secure_borg_command_with_keyfile(
        base_command=base_command,
        repository_path=repository_path,
        passphrase=passphrase,
